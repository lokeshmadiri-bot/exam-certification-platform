...
export function useKeyboardBlock() {
  const { copyBlocked, watermark } = useIntegrity();

  useEffect(() => {
    const handleKeyDown = (e) => {
      // Intercept and prevent Escape key from exiting fullscreen if required
      if (watermark?.fullscreenRequired && (e.key === 'Escape' || e.keyCode === 27)) {
        e.preventDefault();
        e.stopPropagation();
      }
